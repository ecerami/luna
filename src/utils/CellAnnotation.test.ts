...
test("verify active colors", () => {
	...
	// Check at beginning, we should only have one (default) color.
	let defaultColorIndex = annotation.getCategoryIndexColor(CellAnnotation.OTHER_DEFAULT_KEY);
	...
	defaultColorIndex = annotation.getCategoryIndexColor(CellAnnotation.OTHER_DEFAULT_KEY);

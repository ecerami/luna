...
class HexMapPanel extends React.Component<ComponentProps> {
  ...
  getColorValue(dataList: any) {
    ...
    let selectedAnnotationSlug = this.props.lunaState.annotationState
  ...
  getGeneColor(selectedGene: string, dataList: any) {
    ...
    let expressionVector = this.props.lunaState.geneState.geneExpressionValuesMap.get(
      selectedGene
    );
    if (expressionVector) {
      for (let i = 0; i < dataList.length; i++) {
  ...
  setTooltip(info: any, event: any) {
    ...
    if (el != null) {
      if (object) {
        ...
        let selectedAnnotationId = this.props.lunaState.annotationState
          .selectedAnnotationSlug;
